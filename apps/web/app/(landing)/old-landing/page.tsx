import type { Metadata } from "next";
import { Hero, HeroVideoPlayer } from "@/app/(landing)/home/Hero";
import { BasicLayout } from "@/components/layouts/BasicLayout";
import { FeaturesHome } from "@/app/(landing)/home/Features";
import { Privacy } from "@/app/(landing)/home/Privacy";
import { Testimonials } from "@/app/(landing)/home/Testimonials";
import { PricingLazy } from "@/app/(app)/premium/PricingLazy";
import { FAQs } from "@/app/(landing)/home/FAQs";
import { FinalCTA } from "@/app/(landing)/home/FinalCTA";
import { BRAND_NAME, getBrandTitle } from "@/utils/branding";

export const metadata: Metadata = {
  title: getBrandTitle("Home"),
  description:
    "AI-powered email productivity demo for classification, summaries, reply drafts, and safe automation rule previews.",
  alternates: { canonical: "/old-landing" },
};

export default function Home() {
  return (
    <BasicLayout>
      <HeroHome />
      <FeaturesHome />
      <Testimonials />
      <div className="pb-32">
        <PricingLazy />
      </div>
      <Privacy />
      <FAQs />
      <FinalCTA />
    </BasicLayout>
  );
}

function HeroHome() {
  return (
    <Hero
      title="Meet Your AI Email Assistant That Actually Works"
      subtitle={`${BRAND_NAME} organizes your inbox, drafts replies in your voice, and helps you stay on top of your inbox. Never miss an important email again.`}
    >
      <HeroVideoPlayer />
    </Hero>
  );
}
