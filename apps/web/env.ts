...
const parsedEnv = createEnv({
  server: {
    ...
    // Model id for the public /demo classifier (utils/demo/ai). Defaults to
    // a fast/cheap Claude model when unset.
    DEMO_AI_MODEL: z.string().optional(),
    // Global daily cap on live AI calls from the public /demo classifier.
    // Defaults to 300/day (utils/demo/ai/budget.ts) when unset.
    DEMO_AI_DAILY_CALL_LIMIT: z.coerce.number().int().positive().optional(),
